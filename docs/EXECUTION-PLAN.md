...
- **`skill-creator` install** — moved here 2026-08-20 (was Tier 2). Real source found and reviewed: `github.com/anthropics/skills`, `skills/skill-creator/` — official, legitimate (170k+ stars), a genuine trigger-eval toolkit (train/test split, measured pass rates, `improve_description.py` calls `claude -p` as a subprocess). Not installed because: (1) `auditing-skill-triggers` already covers the actual current need, just less quantitatively; (2) it needs a Python dependency (`pyyaml`) — this project's first actual package install, worth a deliberate decision not a side effect; (3) the official `/plugin install` path bundles it with 10+ unrelated skills (`algorithmic-art`, `theme-factory`, `mcp-builder`, etc.) via the `example-skills` plugin — a manual install would need to cherry-pick just `skill-creator`'s own directory. Revisit if `auditing-skill-triggers`'s qualitative judgment calls start missing real issues.
